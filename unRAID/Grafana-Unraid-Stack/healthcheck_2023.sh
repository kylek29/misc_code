...
then
    ...
    then
        ...
        then
            ...
            crashed=$(( $crashed + 1 ))
            touch "${logout_folder}/healthcheck-failure-influxd-at-$(date "+%d.%m.%Y_%T")"
        fi
    ...
    then
        ...
        then
            ...
            crashed=$(( $crashed + 1 ))
            touch "${logout_folder}/healthcheck-failure-loki-at-$(date "+%d.%m.%Y_%T")"
        fi
        echo "[info] Run loki as daemon on port $LOKI_PORT"
        start-stop-daemon --start -b --exec /usr/sbin/loki -- -config.file=${logout_folder}/loki/loki-local-config.yaml
    fi
    ...
    then
        ...
        then
            ...
            then
                ...
                crashed=$(( $crashed + 1 ))
                touch "${logout_folder}/healthcheck-failure-hddtemp-at-$(date "+%d.%m.%Y_%T")"
            fi
            echo "[info] Running hddtemp as daemon due to USE_HDDTEMP set to $USE_HDDTEMP"
            hddtemp --quiet --daemon --file=${logout_folder}/hddtemp/hddtemp.db --listen='127.0.0.1' --port=7634 /rootfs/dev/disk/by-id/ata*
        fi
    fi
    ...
    then
        ...
        then
            ...
            crashed=$(( $crashed + 1 ))
            touch "${logout_folder}/healthcheck-failure-telegraf-at-$(date "+%d.%m.%Y_%T")"
        fi
    ...
    then
        ...
        then
            ...
            crashed=$(( $crashed + 1 ))
            touch "${logout_folder}/healthcheck-failure-promtail-at-$(date "+%d.%m.%Y_%T")"
        fi
        echo "[info] Run promtail as daemon on port $PROMTAIL_PORT"
        start-stop-daemon --start -b --exec /usr/sbin/promtail -- -config.file=${logout_folder}/promtail/promtail.yml
    fi
    ...
    then
        ...
        then
            ...
            crashed=$(( $crashed + 1 ))
            touch "${logout_folder}/healthcheck-failure-grafana-at-$(date "+%d.%m.%Y_%T")"
        fi
